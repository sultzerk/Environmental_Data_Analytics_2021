...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #eliminates those with zero values
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments) #no comments
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% #creating new month column
  select(lakeid:daynum, month, sampledate:comments) %>% #re-ordering columns 
  drop_na(depth) #dropping NA values
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #creating more rows

NTL.nutrient.data.PeterPaul.gathered <- #subset function is similar to filtering subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))#remove concentrations that are NA
count(NTL.nutrient.data.PeterPaul.gathered, nutrient)#counting by nutrients
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) # do the opposite of what we did above
...
```{r}
#if you're not sure what join to use, you can always start with the full join
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) #returning all rows and all columns from these datasets, has NA for missing values
...
LitterFiles #this has all of file names that we need
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID"))#adding variables from each but only number of rows from Litter
...
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum)#fewer rows
