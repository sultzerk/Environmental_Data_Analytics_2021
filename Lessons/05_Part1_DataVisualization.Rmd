...
library(ggridges) #helps you look at distribution of variable over time
...
* geom_errorbar -> adds trendline
* geom_smooth
* geom_line
* geom_area
* geom_abline (plus geom_hline and geom_vline)
* geom_text -> add text to plot or change labels
...
O3plot2 <- ggplot(EPAair) +
  ...
  geom_point(aes(x = Date, y = Ozone), color = "blue") #have to specify color outside of aes
print(O3plot2)
...
PMplot <- 
  ...
  geom_point() #different shapes according to year, sites grouped by color
print(PMplot)

# Separate plot with facets-lets you view multiple plots
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #one plot for each site, 3 rows of plots
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #column with year and row with 3 sites we selected
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #all of data is clumped in left bottom corner except 2 outliers
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #depth is on reverse scale-switch
print(tempvsdepth)

#checking out relationship between tp_ug and tn_ug, does it change based on depth?
NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #trendline for tp_ug and tn_ug
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # color is outline of boxes while "fill" would be the fill of the boxplot with black outline
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #binaxis means you're stacking observations over y, dodge position = observations at same level go to side (no overlay)
print(Nutrientplot4)

# Violin plot-similar to boxplots but violin plots shows density plot (rotated)
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
print(Nutrientplot6) #looks ugly!

# Using a gathered dataset
Nutrientplot7 <-   
  ggplot(PeterPaul.chem.nutrients.gathered) +
  geom_freqpoly(aes(x = concentration, color = nutrient))
print(Nutrientplot7)#legend is automatically created!

# Frequency polygons have the risk of becoming spaghetti plots. Too many lines crossing each other-can't tell anything about data
# See <https://www.data-to-viz.com/caveat/spaghetti.html> for more info.

# Ridgeline plot-shows density of distribution overtime
Nutrientplot6 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(y = nutrient, x = concentration)) +
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #fill colors the inside of the curves
print(Nutrientplot6)
