...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #gray lines/axes on white backgrounds
print(O3plot3)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #increasing font size from base 11
  theme(axis.text = element_text(color = "black"), #changing axis from gray to black
        legend.position = "top") #alternative: legend.position + legend.justification (right, left, center)

# options: call the theme in each plot or set the theme at the start. 

O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #setting theme within plot environment
print(O3plot5)  
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal line with lty=2 (dashed)
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #%b is abbreviated month
...
scales::show_col(colormap(colormap = colormaps$viridis, nshades = 16)) #showing colors for viridis - blue, green, yellow
scales::show_col(colormap(colormap = colormaps$inferno, nshades = 16)) #black, purple,oragne, yellow 
scales::show_col(colormap(colormap = colormaps$magma, nshades = 16)) #very similar to inferno
display.brewer.all(n = 9) #many colors
