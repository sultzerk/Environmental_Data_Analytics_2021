...
Or, you can look at files --> "more"--> "set as working directory"
...
> Answer: The console gives you descriptions of attached packages and potential conflicts. 
...
> ANSWER: in the data --> metadata folder. This relates b/c it's in its own folder and isn't ever changed. No spaces in names of file, and then data, code, and output are stored in different folder. Data --> raw, metadata, or processed. 

```{r exploredata}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) #listed as a "data.frame" as we wanted
colnames(USGS.flow.data) #reading first row as columnn but these names are not informative

# Rename columns from README file
...
str(USGS.flow.data) #shows data frame and all objects
dim(USGS.flow.data) #shows rows and columns of dataset
length(USGS.flow.data) #how many columns

head(USGS.flow.data)
head(USGS.flow.data, 10) #first 10 rows
tail(USGS.flow.data, 5) #bottom 5 rows
USGS.flow.data[30000:30005, c(3, 8, 14)] #row 30,000-30,005 with just column 3,8,14

class(USGS.flow.data$datetime) #class for analyzing single column
...
```{r findNA}
#summary stats show NAs at end of output
...
> Answer: old data that was entered by hand might have NAs. For time series analysis, you never remove NAs.  

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #compare number of rows with original dataset
...
#this converts to date and changes to formatting of y-m-d (internationally used)
```
