...
Look for an API 
Don't do if it's for internal use only, says no data scraping
check on robots.txt (base url/robots.txt)
...
webpage <- read_html('https://www.ncwater.org/WUDC/app/WWATR/report/view/0004-0001/2020') #rvest command
webpage
```
...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% #look for all elements that have these descriptors
  html_text()
the_registrant
...
avg_withdrawals <- webpage %>% 
  ...
  html_text()
avg_withdrawals #character class
...
df_withdrawals <- data.frame("Month" = rep(1:12), #sequence of numbers from 1-12
                             "Year" = rep(2000,12), #12 repeats of the year 2000
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! means set it to this as a variable
...
the_year <- 2014
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 eliminates space between values
...
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility) #returns list of 3 dataframes
...
the_facility_ids_subset <-  sample(the_facility_ids,20) #grabbing 20 random samples
