...
#st as sf converts into spatial feature dataframe, WGS 1984
...
#geometry is a list b/c the counties are polygons
...
st_bbox(counties_sf) #creates spatial bounding box around dataset

#View the data
head(counties_sf)

#Plot the data, colored by area of land in each county
mapView(counties_sf, zcol = "AWATER") #color based on relative amt of water
```
...
#Create a map on top of a map!
mapView(huc8_sf, col.regions="orange")+
  mapView(UpperNeuse, col.regions="purple")
...
ggplot(nc_powerplants_sf) + 
  geom_sf(aes(color = PRIM_FUEL)) #doesn't have nice map background
```
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #need to define FIPS as factor so don't lose impt leading zero
...
wind_HUC12 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
colClasses=c('HUC_12'='factor'))%>%
  mutate(HUC_8=substr(HUC_12,1,8))%>% #taking first 8 characters to make HUC8 column
  group_by(HUC_8)%>%
  summarise(meanWindEnergy=mean(AvgWindEnergy))


#Join to HUC_8 features
huc8_sf_wind <- huc8_sf %>%
  left_join(wind_HUC12, by = c('HUC_8')) #keeping only the ones on the left
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>% #all of counties in this dataset have same STATEFP
  summarize(ALAND = sum(ALAND)) #summarizing total area of land in NC
...
huc_compile <- huc8_sf  %>% 
  ...
  summarize(ACRES=sum(ACRES),
            SQMiles=sum(SQ_MILES))

  
#Map the data
mapView(huc_compile, zcol="ACRES")
...
huc2_utm <-        st_transform(huc_compile, crs = 26917)
```
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] #which counties intersect the Neuse

#Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE))#x=counties_sf_utm, y = intersect polygon

#View the result
myMap + mapview(neuse_intersect_2, alpha.regions = 0)

#Actually intersect the counties (not just select those that intersect)
neuse_counties_sf <- neuse_sf %>% #start with cookie cutter
  st_intersection(counties_sf_utm) #then cookie dough
myMap + mapview(neuse_counties_sf, alpha.regions = 0)

#Update the area the features, now that some are clipped (they don't automatically update!)
...
triCo_centroids_2km <- st_buffer(triCo_centroids, 2000) #2000m buffer
myMap <- myMap + geom_sf(data = triCo_centroids_2km, color = 'orange', fill=NA)
myMap

#Buffer the counties inward 2km
triCo_in2km <- st_buffer(triCo, -2000) #buffer from the outline of the county
...
#Combine the centroids into one feature and draw voronoi polygons-similar to service areas for monitoring sites
...
#Map the results
mapView(counties_sf[resultMask,]) #this shows the county that the point is in!
...
resultMask_30m <- st_intersects(counties_sf_utm,
                                site_30km,
                                sparse = FALSE)

selCounties_30km <- counties_sf_utm[resultMask_30m,]
...
ggplot()  +
  ...
  geom_sf(data = state_sf_utm, color='red',size=2) + #this polygon outline of the state covers up everything else
...
#Create an lattice view of the two leaflet maps-2 side by side maps
latticeview(m1, m2)
