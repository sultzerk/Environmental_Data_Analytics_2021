...
str(USGS.flow.data) #looks at structure of data-classes
dim(USGS.flow.data)
...
class(USGS.flow.data$datetime) #above changed to "date" class
...
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar() #aes=aesthetic, what data are we plotting, x = categorical variable

ggplot(na.omit(USGS.flow.data), aes(x = discharge.mean.approval)) +
  geom_bar() #if don't want to include any rows with NA
```
...
#many more bins-smoother data
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#fewer bins-chunkier data, default bins are 30
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#zooms in- scale x continuous sets max of xlim
...
#color can be set to categorical value
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#x indicates categories, y is continuous numerical data
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#slicing data based on gage.ht.mean
#at low values of gage ht- tight value vs. IQR goes up as gage ht increases
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#showing how much data is within IQR range-gets fatter
ggplot(USGS.flow.data) +
...
> Answer: bar plot: categorical variables
histogram: continuous numerical data (just one variable)
frequency polygon: continous numerical data (1+ variables)
box and whisker: continuous numerical data, if want to separate by categories
violin: hybrid of box and whisker and frequency polygon
scatterplot: continuous numerival variable vs. continuous numerical variable-how do they directly compare?

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: trends in different variables and where the values are concentrated- what's most prevalent and why


How can multiple options for data exploration inform our understanding of our data?

> Answer: depending on what question we want to answer, different options can help us understand specific answers
