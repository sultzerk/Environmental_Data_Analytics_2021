...
vector1[3] # 
vectorx <- c(1,"one") #defaults to broadest category-character

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #converting vector into matrix 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #numbers increase across rows not down columns
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line, gives row and column names
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #

matrix1[4, ] #everything in row 4
matrix1[ , 3] #everything in column 3
matrix1[c(12, 14)] #giving 12th and 14 value if matrix arranged like vector
matrix1[c(12:14)] #giving 12th-14 values if matrix arranged like vector
matrix1[2:4, 1:3] #2-4th row and 1-3rd column
...
# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 #lists can be different modes
list1[[2]] #returns to numeric not "list", use "class" to confirm
...
> Answer: multiple dimensions appear in "data" while one dimensional data appears in "values"

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: You can view columns in data frames. 
...
```{r}
High <- c(53,55,54,49,53,56,53,42,36,34)
Low <- c(25,41,32,32,28,36,26,16,22,21)
```
...
```{r}
Temperatures <- data.frame(High,Low,High_C,Low_C)
names(Temperatures) <- c("HighFahrenheit", "LowFahrenheit", "HighCelsius", "LowCelsius")
```
...
sd(Temperatures$LowFahrenheit) #can't do sd on dataframe- have to do it on vector
summary(Temperatures$HighFahrenheit)
```
