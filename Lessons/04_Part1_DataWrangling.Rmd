...
str(NTL.phys.data) #look at class of columns-date is not a "date" class
dim(NTL.phys.data)
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #make matrix where lake depth = 0
...
> Answer: row numbers change and aren't specific to the dataset
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #goes up with depth 0 as starting pt
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #desc is short for descending
...
#east long lake and hummingbird lake, depths of 0 or 0.5 and years from 1995-2002
```
### Select
...
```{r}
#creating subset with lakename, sampledate, depth, temperature
...
```{r}
#adding new column to specific dataset
...
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #first row and last row
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate))#use first and last function
```
