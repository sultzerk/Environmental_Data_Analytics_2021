...
#not a balanced experiment b/c some sites have more values than others
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) #follow normal dist.
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #follow normal dist.
...
#data does not follow a normal distribution- curved around straight line here
...
> First, reorder lists to show a to b to c to d
...
#holy mackerel, this is a lot of combinations of interactions of variables!
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot()+
  labs(x="NLCD Class", y= "Dry Mass(g)", color="Functional Group")+
   stat_summary(geom = "text", fun = max,position = position_dodge(0.8),vjust=-3,
               label = c("d", "d", "d", "a", "d", "d", 
                         "d", "d", "d", "d", "d", "b", 
                         "d", "d", "d", "d", "d", "d",
                         "cd", "bc", "d", "d", "d", "d")) #labels are not adjusted right
