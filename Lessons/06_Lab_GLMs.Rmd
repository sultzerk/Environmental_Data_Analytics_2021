...
> Answer: H0: mu >= 50 ppm, Ha: mu < 50 ppm
...
EPAair.subsample <- sample_n(EPAair, 5000) #shapiro can only run on 5000 records

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #significant p-value: reject Ho so data are not normally distributed (likelihood of normal data is small)
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() #not wayyy away from normality
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)#points go away from the tails

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample #reject null and say that our mean (statistically speaking)is less than 50
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #gets rid of gap on bottom of graph
print(Ozone.plot)
```
...
> On average, ozone measured in NC is less than 50 ppm (t-test, df = 6829, p<0.01). DF shows how many samples you have
...
ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() #histograms follow same general humps but 2018 < 2019

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
O3.twosample
O3.twosample$p.value #mean values are different
...
the_model <- lm(data = EPAair, PM2.5~Ozone)
summary(the_model) #R2 is only 13% but p-value is significant
