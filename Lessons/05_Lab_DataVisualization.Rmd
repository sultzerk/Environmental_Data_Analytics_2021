...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", pi,",", R[2],'=0.6'))) #inputting math expression into graph
print(Plot_text)
...
PLot1 <- ggplot(subset(PeterPaul.chem.nutrients, depth==0), 
                aes(x=daynum, 
                    y=temperature_C))+
  geom_point(aes(color=year4), shape=8, alpha=0.5)+
...
Plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C))+
  geom_point(aes(color=depth),dotsize=0.5, alpha=0.5)+
...
Plot3 <- ggplot(EPAair,aes(x=PM2.5,y=Ozone))+
...
# 4. 
# Create several types of plots depicting PM2.5, divided by year. -use as.factor
