...
#Read Eno river data, this data is daily
...
#Read wind speed data, this data is monthly
...
#define starting point, when the first observation is, frequecny of observations-daily data
...
#only one NA in average wind per month (AWND)
...
#na.approx = used to interpolate missing values using linear or spline interpolation type within zoo package
...
#missing observation filled in in red with interpolation
```
...
# Run SMK testo- this is the only test that can handle seasonal data
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts)

# Inspect results
wind_data_trend1 #p-value is small- reject that data is stationary so we have a trend
...
#results of tau for each season of year
#some seasons there is a stronger tendency to decrease (higher negative S value)
...
> Answer: negative slope showing decreased trend of average wind speed over time
