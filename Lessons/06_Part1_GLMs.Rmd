...
editor_options: 
  chunk_output_type: console #output going to the console
---
...
#as depth increases, light level decreases
#coefficients are statistically different than 0
...
So, we see there is a significant negative correlation between irradiance and depth (lower light levels at greater depths), and that this model (depth) explains about 31 % of the total variance in irradiance. Let's visualize this relationship and the model itself. 
...
par(mfrow = c(1,1))
#these plots are messed up by one major residual!!
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #gets rid of outlier
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth)
summary(irradiance.regression2)
#residual standard error is reduced and depth now explains 73% (up from 31%)

par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression2)
par(mfrow = c(1,1))
#these look MUCH better without the outlier
...
#relationship is different than zero for 2 xs individually, se = 6.4, r2=30%
...
plot(TPregression)
par(mfrow = c(1,1))
#first line is not flat- so something is missing
#normal Q-Q plot isn't bad but there's some outliers 
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #creating correlation matrix for all nutrients
#what variables are highly correlated with each other?
corrplot(PeterPaulCorr, method = "ellipse")
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #mix of number and ellipses
#which should I add onto model?
```
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #consider all nutrients as explanatory variables

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug) #this regression had lowest AIC
summary(TPmodel)
#r2 actually decreased which is unusual! but SE also decreased
```