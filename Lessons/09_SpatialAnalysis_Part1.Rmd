...
#coord_equal helps plot what it looks like on a map- looks similar to state of NC
```
...
#Show the map- this is interactive and you can zoom
myMap
...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>% #tiles are the backgrounds
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius=(~meanPM*2),
                   stroke = FALSE, 
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`)) #when you click on the circle marker, it'll show the name
#Show the map
myMap
#you can save the map - export- save as webpage
```
...
#Plot the geometry...
plot(sf_EPAair_PM_avg$geometry)#looks like the shape of NC

#Plot everything
plot(sf_EPAair_PM_avg)#shows site name, county, meanPM, and maxPM separately
...
mapview(sf_EPAair_PM_avg, 
        zcol='meanPM', #color of mean
        cex='maxPM') #size of max
...
Durham_buffer <- sf_EPAair_PM_avg %>% 
  ...
  st_buffer(0.1) #0.1 unit which here is 0.1 degree
...
sf_EPAair_PM_avg_UTM <- st_transform(sf_EPAair_PM_avg, crs=26917)#values are now in meters
...
mapView(DA_UTM_buffer)                         # View the results-looks circular
```
...
grainger_sfg <- st_point(c(-78.9427,36.00465)) #didn't specify coordinate system

#Examine this item
class(grainger_sfg) #single geometry

#Convert to a sfc object, adding a coordinate reference system
grainger_sfc <- st_sfc(grainger_sfg, crs=4326)

#Examine this item
class(grainger_sfc) #geom and reference info
...
#Check the EPSG of our transformed object; it should equal 26917 (the one for UTM zone 17N)
st_crs(grainger_sfc_utm)$epsg

#Now, try computing distance again
the_distance <- st_distance(x = sf_DurhamArmory,
                            y = grainger_sfc_utm,
                            by_element = TRUE)#keeps it to a number, not a matrix
the_distance #4692 meters away
