...
Temp.plot <- ggplot(BRA_complete, aes(x=Date, y=Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm")
Temp.plot #slight upward trend

Rain.plot <- ggplot(BRA_complete, aes(x=Date, y=Rainfall))+
  geom_line()+
  geom_smooth(method = "lm")
Rain.plot #pretty flat or perhaps stationary
...
temp_ts <- ts(BRA_complete$Temperature_C,
                   start=c(f_year,f_month),
                   frequency=12) #measured monthly
rain_ts <-ts(BRA_complete$Rainfall,
                   start=c(f_year,f_month),
                   frequency=12) #measured monthly

#ts requires a certain number of steps between each cycle- that's why we interpolate any missing data
```
...
The `stl` function decomposes the time series object into its component parts. We must specify that the window for seasonal extraction is either "periodic" (according to your data, like 12) or a specific number of at least 7. The decomposition proceeds through a loess (locally estimated scatterplot smoothing) function.
...
temp_components <- mutate(temp_components,
         Temperature_C= BRA_complete$Temperature_C,     
        Date = BRA_complete$Date)


rain_components <- as.data.frame(rain_decomp$time.series[,1:3])
...
temp.trend.plot <- ggplot(temp_components) +
  geom_line(aes(y = Temperature_C, x = Date),  size = 0.25, color="grey") +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff",size=1)
print(temp.trend.plot)

rain.trend.plot <- ggplot(rain_components) +
  geom_line(aes(y = Rainfall, x = Date),  size = 0.25, color="grey") +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff",size=1)
print(rain.trend.plot)
...
rain_data_trend2 <- smk.test(rain_ts)

temp_data_trend <- smk.test(temp_ts)

#inspect results
rain_data_trend1 #trend is significant! small p-value
summary(rain_data_trend1)
summary(rain_data_trend2) #which months are significantly different

temp_data_trend
summary(temp_data_trend)#trend is significant
...
ts_rain_noseason <- ts(data=rain_components$Rainfall-rain_components$seasonal,
                       start=c(f_year, f_month),
                       frequency = 12)

#apply Mann Kendall
mk_rain <- trend::mk.test(ts_rain_noseason)
mk_rain
#low p-value so there's a trend
