...
> Answer: We didn't use a piecewise constant or spline function because there were NAs directly next to each other.  It was better to just use the "connect the dots" approach since ppms are continuous. spline won't be accurate b/c it's quadratic so it might be higher than normal and piecewise- how do we know which value is closest to it?
...
> Answer: The seasonal Mann-Kendall is the most appropriate trend analysis here since there's an obvious seasonal component to the data. If there wasn't a seasonal component, we would consider one of the other tests like linear regression, regular Mann-Kendall, or Spearman Rho. 
...
Mean.month.ozone.plot <-
  ...
  geom_line()+
  geom_point()+
  labs(title="Garinger High School Mean Monthly Ozone Concentrations", y="Monthly Mean Ozone Concentration (ppm)")+
...
> Answer: From our analysis, we can say that ozone concentrations have changed over the 2010s at Garinger High School (p)
